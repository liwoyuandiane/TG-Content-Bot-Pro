...
class ClientManager:
    ...
    async def _handle_userbot_start_error(self, error: Exception, session_string: str):
        ...
        if any(keyword in error_msg for keyword in session_error_keywords):
            logger.warning("检测到无效SESSION，建议用户重新生成")
            # 不自动删除数据库中的SESSION(避免误删其他用户的记录),
            # 仅提示用户重新生成。settings.SESSION 置空防止后续重试。
    ...
    async def restart_bot(self) -> bool:
        ...
        try:
            ...
            self._create_pyrogram_bot()
            await self._start_pyrogram_bot()
            ok = self.bot is not None and self.bot.is_connected
            if ok:
                logger.info("bot 客户端重启成功")
            else:
                logger.error("bot 客户端重启后未连接")
            return ok
        except Exception as e:
            logger.error(f"重启 bot 客户端失败: {e}", exc_info=True)
            return False
